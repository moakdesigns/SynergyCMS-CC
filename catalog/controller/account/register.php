...
class ControllerAccountRegister extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_account_member->addMember($this->request->post);

			$this->member->login($this->request->post['email'], $this->request->post['password']);
		...
		if (is_array($this->config->get('config_member_group_display'))) {
			$member_groups = $this->model_account_member_group->getMemberGroups();

			foreach ($member_groups as $member_group) {
				if (in_array($member_group['member_group_id'], $this->config->get('config_member_group_display'))) {
					$this->data['member_groups'][] = $member_group;
				}
			}
		}
	...
	protected function validate() {
		...
		if ($this->model_account_member->getTotalMembersByEmail($this->request->post['email'])) {
		...
		if (isset($this->request->post['member_group_id']) && is_array($this->config->get('config_member_group_display')) && in_array($this->request->post['member_group_id'], $this->config->get('config_member_group_display'))) {
			$member_group_id = $this->request->post['member_group_id'];
		} else {
			$member_group_id = $this->config->get('config_member_group_id');
		}

		$member_group = $this->model_account_member_group->getMemberGroup($member_group_id);
		...
		if ($country_info) {
			if ($country_info['zipcode_required'] && (utf8_strlen($this->request->post['zipcode']) < 2) || (utf8_strlen($this->request->post['zipcode']) > 10)) {
