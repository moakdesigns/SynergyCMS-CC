...
class ControllerCatalogReview extends Controller {
	...
	protected function getList() {
		...
		$this->data['sort_post'] = $this->url->link('catalog/review', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, 'SSL');
	...
	protected function getForm() {
		...
		if (isset($this->error['post'])) {
			$this->data['error_post'] = $this->error['post'];
		} else {
			$this->data['error_post'] = '';
		}
		...
		if (isset($this->request->post['post_id'])) {
			$this->data['post_id'] = $this->request->post['post_id'];
		} elseif (!empty($review_info)) {
			$this->data['post_id'] = $review_info['post_id'];
		} else {
			$this->data['post_id'] = '';
		}
		...
		if (isset($this->request->post['post'])) {
			...
			$this->data['post'] = $review_info['post'];
		} else {
			$this->data['post'] = '';
		}
