...
class ControllerMembersMember extends Controller { 
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_member_group_id'])) {
				$url .= '&filter_member_group_id=' . $this->request->get['filter_member_group_id'];
			}
			...
			$this->redirect($this->url->link('members/member', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_member_group_id'])) {
			$filter_member_group_id = $this->request->get['filter_member_group_id'];
		} else {
			$filter_member_group_id = null;
		}
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('members/member', 'token=' . $this->session->data['token'] . $url, 'SSL'),
			'separator' => ' / '
		);

		$this->data['approve'] = $this->url->link('members/member/approve', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['insert'] = $this->url->link('members/member/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['delete'] = $this->url->link('members/member/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		$data = array(
			...
			'filter_member_group_id' => $filter_member_group_id, 
		...
		$member_total = $this->model_members_member->getTotalMembers($data);
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('members/member/update', 'token=' . $this->session->data['token'] . '&member_id=' . $result['member_id'] . $url, 'SSL')
			);
		...
		$this->data['sort_name'] = $this->url->link('members/member', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$this->data['sort_email'] = $this->url->link('members/member', 'token=' . $this->session->data['token'] . '&sort=c.email' . $url, 'SSL');
		$this->data['sort_member_group'] = $this->url->link('members/member', 'token=' . $this->session->data['token'] . '&sort=member_group' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('members/member', 'token=' . $this->session->data['token'] . '&sort=c.status' . $url, 'SSL');
		$this->data['sort_approved'] = $this->url->link('members/member', 'token=' . $this->session->data['token'] . '&sort=c.approved' . $url, 'SSL');
		$this->data['sort_ip'] = $this->url->link('members/member', 'token=' . $this->session->data['token'] . '&sort=c.ip' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('members/member', 'token=' . $this->session->data['token'] . '&sort=c.date_added' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('members/member', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');

		$this->data['pagination'] = $pagination->render();

		$this->data['filter_name'] = $filter_name;
		$this->data['filter_email'] = $filter_email;
		$this->data['filter_member_group_id'] = $filter_member_group_id;
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['member_id'])) {
			$this->data['action'] = $this->url->link('members/member/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('members/member/update', 'token=' . $this->session->data['token'] . '&member_id=' . $this->request->get['member_id'] . $url, 'SSL');
		}

		$this->data['cancel'] = $this->url->link('members/member', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	// 	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission('modify', 'members/member')) { 
	// 		$this->model_members_member->addTransaction($this->request->get['member_id'], $this->request->post['description'], $this->request->post['amount']);
	...
	// 	$results = $this->model_members_member->getTransactions($this->request->get['member_id'], ($page - 1) * 10, 10);
	...
	// 	$this->data['balance'] = $this->currency->format($this->model_members_member->getTransactionTotal($this->request->get['member_id']), $this->config->get('config_currency'));

	// 	$transaction_total = $this->model_members_member->getTotalTransactions($this->request->get['member_id']);
	...
	// 	$pagination->limit = 10; 
	// 	$pagination->text = $this->language->get('text_pagination');
	// 	$pagination->url = $this->url->link('members/member/transaction', 'token=' . $this->session->data['token'] . '&member_id=' . $this->request->get['member_id'] . '&page={page}', 'SSL');

	// 	$this->data['pagination'] = $pagination->render();

	// 	$this->template = 'members/member_transaction.tpl';		
	...
	// 	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->user->hasPermission('modify', 'members/member')) { 
	// 		$this->model_members_member->addReward($this->request->get['member_id'], $this->request->post['description'], $this->request->post['points']);
	...
	// 	$results = $this->model_members_member->getRewards($this->request->get['member_id'], ($page - 1) * 10, 10);
	...
	// 	$this->data['balance'] = $this->model_members_member->getRewardTotal($this->request->get['member_id']);

	// 	$reward_total = $this->model_members_member->getTotalRewards($this->request->get['member_id']);
	...
	// 	$pagination->limit = 10; 
	// 	$pagination->text = $this->language->get('text_pagination');
	// 	$pagination->url = $this->url->link('members/member/reward', 'token=' . $this->session->data['token'] . '&member_id=' . $this->request->get['member_id'] . '&page={page}', 'SSL');

	// 	$this->data['pagination'] = $pagination->render();

	// 	$this->template = 'members/member_reward.tpl';		
