...
class ControllerLocalisationCountry extends Controller {
	...
	protected function getForm() {
		...
		$this->data['entry_zipcode_required'] = $this->language->get('entry_zipcode_required');
		...
		if (isset($this->request->post['zipcode_required'])) {
			$this->data['zipcode_required'] = $this->request->post['zipcode_required'];
		} elseif (!empty($country_info)) {
			$this->data['zipcode_required'] = $country_info['zipcode_required'];
		} else {
			$this->data['zipcode_required'] = 0;
		}
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $country_id) {
			...
			$site_total = $this->model_setting_site->getTotalSitesByCountryId($country_id);

			if ($site_total) {
				$this->error['warning'] = sprintf($this->language->get('error_site'), $site_total);
			}

			$address_total = $this->model_members_member->getTotalAddressesByCountryId($country_id);
