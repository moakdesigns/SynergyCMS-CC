...
if (isset($_SERVER['HTTPS']) && (($_SERVER['HTTPS'] == 'on') || ($_SERVER['HTTPS'] == '1'))) {
	$site_query = $db->query("SELECT * FROM " . DB_PREFIX . "site 
...
if ($site_query->num_rows) {
	$config->set('config_site_id', $site_query->row['site_id']);
} else {
	$config->set('config_site_id', 0);
}
		
// Settings
$query = $db->query("SELECT * FROM " . DB_PREFIX . "setting 
	WHERE site_id = '0' 
	OR site_id = '" . (int)$config->get('config_site_id') . "' 
